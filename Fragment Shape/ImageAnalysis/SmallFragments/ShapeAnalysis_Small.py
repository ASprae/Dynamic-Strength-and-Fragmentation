...
samples = ['SeeSst_014', 'SeeSst_022', 'SeeSst_028', 'TaQu_006', 'TaQu_008', 'TaQu_019', 'SaLi_002', 'SaLi_008', 'SaLi_014', 'CaMa_004', 'CaMa_009', 'CaMa_016']
images = [4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4]

minsize = [0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05] # cm

pixpercm = [690.7, 703.5, 708.0, 686.1, 699.0, 703.1, 705.4, 713, 711.8, 711.2, 712.2, 717.3]

material = ['SeeSst', 'SeeSst', 'SeeSst', 'TaQu', 'TaQu', 'TaQu', 'SaLi', 'SaLi', 'SaLi', 'CaMa', 'CaMa', 'CaMa']
erate = [145.9, 48.1, 280.4, 183.9, 98.5, 40.1, 58.7, 141.8, 345.5, 91.9, 126.7, 68.5]
erate_sig = [8.8, 5.1, 42.7, 5.8, 10.2, 0.1, 3.5, 24.4, 83.2, 20.1, 19.8, 15.1]
...
for s in samples:
    ...
    for im in np.arange(1, images[n] + 1, 1):
    
        print ("================= {} =================".format(im))
    
        area, perim, maj, min, C, AR = np.genfromtxt('{}/{}_{:03d}.csv'.format(s, s, im), delimiter = ',', skip_header = 1, usecols = (1, 4, 5, 6, 8, 14), unpack = True)
    ...
    min = np.concatenate(MIN)
    C = np.concatenate(CIRC)
    AR = np.concatenate(AspR)
    ...
    diam_filtered = d_equi[d_equi < 2 * minsize[n]]
    diam_filtered = diam_filtered[diam_filtered > 0.5 * minsize[n]]
    
    kde_min = stats.gaussian_kde(diam_filtered)
    span_min = np.linspace(0.5 * minsize[n], 2 * minsize[n], num = 1000)
    ...
    fig.savefig('{}/{}/F2_{}_size.png'.format(s, fname, s))
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig('{}/{}/F3_{}_shape.png'.format(s, fname, s))
    ...
    fig.tight_layout()
    fig.savefig('{}/{}/F4_{}_shape_area.png'.format(s, fname, s))
    plt.close(fig)
    ...
    fig.savefig('{}/{}/F5_{}_shape_corr.png'.format(s, fname, s))
    plt.close(fig)
...
for i in unique_M:
    ...
    ax1.errorbar(rate, Cmean, xerr = rate_sig, yerr = Csd, fmt = 's', c = color_dict[i], label = i, capsize = 5)
    ax2.errorbar(rate, ARmean, xerr = rate_sig, yerr = ARsd, fmt = 's', c = color_dict[i], label = i, capsize = 5)
    
    ax1b.errorbar(rate, Cskew, xerr = rate_sig, fmt = 's', c = color_dict[i], label = i, capsize = 5)
    ax2b.errorbar(rate, ARskew, xerr = rate_sig, fmt = 's', c = color_dict[i], label = i, capsize = 5)
    
    ax1c.errorbar(rate, Cr, xerr = rate_sig, fmt = 's', c = color_dict[i], label = i, capsize = 5)
    ax2c.errorbar(rate, ARr, xerr = rate_sig, fmt = 's', c = color_dict[i], label = i, capsize = 5)
    
ax1.legend(loc = 4)

Sfig01.tight_layout()
Sfig01.savefig('OVERALL_Summary.png')
...
for pc in vp1['bodies']:
    ...
    pc.set_facecolor(color_dict[mat_label])
    pc.set_edgecolor('k')
...
Sfig02.tight_layout()
Sfig02.savefig('OVERALL_Violin_Summary.png')
